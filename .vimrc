...
let g:python_host_prog = '/home/example-org/.pyenv/versions/2.7.17/bin/python'
...
:	let repo = system("git config -l | grep 'origin.url' | grep -oP '(?<=git@gitlab.example.com:|http://gitlab.example.com/)(.*)(?=.git)' | tr -d '\n' ")
:	let relativepath = "./" . expand("%")
:	let branch = "master"
:	let filepath = system('git ls-files --full-name ' . l:relativepath)
:	let @+ = "http://gitlab.example.com/" . l:repo . "/blob/" . l:branch . "/" . l:filepath
