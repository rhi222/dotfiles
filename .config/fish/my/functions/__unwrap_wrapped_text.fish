# 折返しで改行が混入したテキストを1行へ戻す（第1引数 → stdout）
#
...
#
# **入力は引数で受ける。stdinは読まない。** 初版はstdinを `cat` で読んでいたが、
# `win32yank | __unwrap_wrapped_text` のパイプライン要素にすると、fishがパイプの
# write端を握ったままにするためcatにEOFが届かず、key binding経由の呼び出しで
# shellごとデッドロックした。string系builtinは引数なしで呼ぶとstdinへ後戻りするので、
# 変数の参照は必ずquoteして空でも1引数として渡す。
#
...
function __unwrap_wrapped_text --description 折返し由来の改行を畳んで1行にする
    ...
    for line in (string split \n -- "$argv[1]")
        ...
        if test -z "$result"
            set result (string trim --left -- "$trimmed")
        else if string match -qr '^[ \t]' -- "$trimmed"
            # ① 単語境界で切れている。インデントを落として空白1個でつなぐ
            set result "$result "(string trim --left -- "$trimmed")
        else
