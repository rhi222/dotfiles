...
| Linearへの起票を明示した依頼                       | Linear（team `NSY`） | `/linear-add`                           |
| Jiraへの起票・書き込みを明示した依頼               | Jira                 | 指示された操作だけを行う                |
| 「起票して」「チケット作って」「タスクを積んで」   | 未確定               | GitHub・Jira・Linearのどれかを確認する   |
...
- Jiraへの起票・書き込みは、ユーザーがその操作を明示した場合に限る
