...
/my-commit-message --lang ja
/my-commit-message --lang en

# Breaking Change を検出
/my-commit-message --breaking
```
...
- コミットコマンドを `win32yank.exe` に渡す際は、メッセージ出力とは別プロセスで実行すること
...
printf 'git commit -m "%s"' "$COMMIT_MESSAGE" | win32yank.exe -i
