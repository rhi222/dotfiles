...
本日の日報を完成させます。
日報のパス: ~/Obsidian/02_Daily/nippo.$(date +%Y-%m-%d).md）
...
- ~/Obsidian/02_Daily//nippo-report\_タイムスタンプ.md として保存

4. **ファイルの整理**
   - ~/Obsidian/02_Daily/ の日報ファイルを削除（バックアップ後）

!NIPPO*FILE="~/Obsidian/02_Daily//nippo.$(date +%Y-%m-%d).md" && if [ -f "$NIPPO_FILE" ]; then
TIMESTAMP=$(date '+%Y%m%d*%H%M%S')
REPORT*FILE="~/Obsidian/02_Daily//nippo-report*${TIMESTAMP}.md"
...
!GOALS_FILE="~/Obsidian/02_Daily//nippo-goals.txt" && if [ -f "$GOALS_FILE" ]; then echo "=== 目標設定 ==="; cat "$GOALS_FILE"; echo ""; else echo "=== 目標設定 ==="; echo "目標が設定されていません。/nippo-goals で設定してください。"; echo ""; fi
