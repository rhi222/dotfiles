...
現在の日報ファイル（~/Obsidian/02_Daily//nippo.$(date +%Y-%m-%d).md）の内容を表示し、編集状況を確認できます。
...
!NIPPO_FILE="~/Obsidian/02_Daily//nippo.$(date +%Y-%m-%d).md" && echo "📋 日報内容確認 - $(date +%Y年%m月%d日)" && echo "================================" && if [ -f "$NIPPO_FILE" ]; then
...
fi && echo "" && echo "📅 過去の日報一覧（直近5日分）:" && echo "--------------------------------" && ls -lt ~/Obsidian/02_Daily//nippo.*.md 2>/dev/null | head -5 | while read line; do
