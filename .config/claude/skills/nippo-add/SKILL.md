...
続けて、**Linearの `role:manager` タスクからCodexに任せる候補を提示し、承認を取って
EMレーンへ投入**し、`## AIに任せる候補` セクションを書いてください。あわせて、前回までの
成果物で未回答の確認質問を `## AIからの質問` セクションに出してください。
候補の選び方・承認の取り方・投入コマンド・未回答質問の拾い方は `em-delegation.md`
サポートファイルに従います。
**承認されなかったものはstateを動かしません。** 対象が無ければセクションごと省略します。
Linear や `em-dispatch.sh` にアクセスできない場合も日報作成は中断しません。
