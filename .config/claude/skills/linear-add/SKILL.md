...
- **PRを伴う課題は1PRでも親子に分ける**（PRの本数はあとから増えるため）
...
自分のopen draft PRを拾って起票するときにも使う（`元URL:` 付き）。まだPRが無いものに
同じ名前を付けると、タイトルからdispatchモードを判別できなくなる。

「仕上げ」の主語はAIであって人間ではない。人間がやるのは `AI Queued` への遷移（委譲の判断）と
`My Review` 以降（マージの判断）の両端で、真ん中の実装はdispatchが担う。
