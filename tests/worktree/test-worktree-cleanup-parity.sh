#!/bin/bash
# Shell 版 worktree-cleanup.sh と Go 版 dotctl worktree cleanup の出力を比較する。
#
# **移植で文言も桁も変えない**という方針の検査。実リポジトリを1つ作り、
...
sh_exec=$(env WORKTREE_CLEANUP_ROOTS="$SH_ROOT" WORKTREE_CLEANUP_PR_STATE_CMD="$STUB" \
