...
何がどう見えるか・どのキーで何が起きるかはあちらの表にあり、ここには
**なぜその実装に落ちたか**を置く。herdr 0.8.2 時点の話。

## タブ行のステータス

- **時刻・agent usage・マシンリソースを別々の command にしている。** 表示順を
  時刻 → usage → リソースに固定し、グループ間は `  │  `、リソース内は `  ·  ` で区切る。
...
- **`clock.sh` と `status.sh` は外部コマンドを1つも呼ばない。** date / awk / grep / cut を素直に使った初版は
  実測 53ms/回で、1秒間隔だと1コアの5%を常時食う。bash 組み込みだけに寄せて各スクリプトを
  数msに抑えている。
