...
#
# 参照の書き方は1つではない。`bash scripts/x.sh`、`$DOTFILES_DIR/scripts/x.sh`、
# `$HOME/scripts/x.sh`（~/scripts は repo の scripts/ への symlink）、
# 同一ディレクトリ内の `$SCRIPT_DIR/x.sh` が実在する。**逆に
...
TARGET="$SCRIPT_DIR/ref-check.sh"
...
setup
make_target scripts/alpha.sh
make_referrer AGENTS.md '確認は `bash scripts/alpha.sh`。'
out=$(run_check)
rc=$?
...
make_target scripts/alpha.sh
make_referrer AGENTS.md '確認は `bash scripts/alpha.sh`。' \
  '掃除は `bash scripts/ghost.sh`。'
out=$(run_check)
rc=$?
check "dangling があれば失敗で返す" "1" "$rc"
check "壊れたパスを名指しする" "yes" "$(has 'scripts/ghost.sh' "$out")"
check "参照元を file:line で出す" "yes" "$(has 'AGENTS.md:2' "$out")"
check "生きている参照は出さない" "no" "$(has 'scripts/alpha.sh' "$out")"
teardown
...
make_referrer docs/bootstrap.md \
  'bash "$DOTFILES_DIR/scripts/alpha.sh"' \
  '$HOME/scripts/alpha.sh' \
  '~/scripts/alpha.sh' \
  '"$REPO_ROOT/scripts/alpha.sh"'
out=$(run_check)
rc=$?
...
make_referrer docs/bootstrap.md 'bash "$DOTFILES_DIR/scripts/ghost.sh"'
out=$(run_check)
rc=$?
check "prefix 付きの dangling は落とす" "1" "$rc"
check "正規化後のパスで報告する" "yes" "$(has 'scripts/ghost.sh' "$out")"
teardown
...
setup
make_target scripts/alpha.sh
make_target scripts/lib/helper.sh
make_referrer scripts/caller.sh \
  'source "$SCRIPT_DIR/lib/helper.sh"' \
  'bash "$SCRIPT_DIR/alpha.sh"'
out=$(run_check)
rc=$?
...
make_target scripts/alpha.sh
make_referrer tests/worktree/test-alpha.sh 'TARGET="$SCRIPT_DIR/alpha.sh"'
out=$(run_check)
rc=$?
check "別ディレクトリからの \$SCRIPT_DIR は解決先が変わる" "1" "$rc"
check "解決後のパスで報告する" "yes" "$(has 'tests/worktree/alpha.sh' "$out")"
teardown

echo "== allowlist =="

setup
make_referrer scripts/test-skill-audit.sh 'cat >"$d/scripts/run.sh" <<EOF'
allow '# フィクスチャ本文に出てくる架空のパス' 'scripts/run.sh'
out=$(run_check)
rc=$?
...
make_referrer AGENTS.md '`bash scripts/ghost.sh`'
out=$(run_check)
rc=$?
...
make_target scripts/secret-patterns.txt.example
make_referrer dotfilesLink.sh 'cp "$DOTFILES_DIR/scripts/secret-patterns.txt.example" "$p"'
out=$(run_check)
rc=$?
...
printf '\x00\x01scripts/ghost.sh\x00' >"$REPO/blob.bin"
out=$(run_check)
rc=$?
