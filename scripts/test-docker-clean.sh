...
assert_not_contains "wbc-booking-record_db_test" "$out" "--excluded は表示対象を出さない"
...
assert_contains "docker stop wbc-booking-record_db_test" "$out" "コピペ用の停止コマンドを出す"
...
assert_not_contains "docker stop" "$out" "0件なら停止コマンド例を出さない"
