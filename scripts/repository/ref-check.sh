...
#
# 例外は scripts/repository/ref-check-allow.txt で宣言する（書式は1行1パス、glob 可）。
#
...
#     （旧名はcompat manifestからHOME側へlink）
...
# ~/scripts 直下の旧名はrepo内に実体を残さず、compat-links.txtから正規pathへ解く。
# manifest側のtargetが消えていれば互換参照もdanglingとして検出する。
is_compat_ref() {
  local path="$1" old target
  [ -f "$REPO_ROOT/scripts/compat-links.txt" ] || return 1
  [ "${path#scripts/}" != "$path" ] || return 1
  while IFS="|" read -r old target; do
    [[ -n "$old" ]] || continue
    [[ "$old" == "#"* ]] && continue
    [ "$path" = "scripts/$old" ] || continue
    [ -e "$REPO_ROOT/scripts/$target" ]
    return
...
while IFS= read -r hit; do
  ...
  [ "$file" = "scripts/repository/ref-check-allow.txt" ] && continue
