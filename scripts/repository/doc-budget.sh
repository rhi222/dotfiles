...
#
#   bash scripts/repository/doc-budget.sh            # 作業ツリーを検査
#   bash scripts/repository/doc-budget.sh --staged   # ステージ済みの内容を検査（pre-commit hook から）
#
# 予算は scripts/repository/doc-budget.txt で宣言する。書式は1行1ファイルで
...
if [ ! -f "$DECL" ]; then
  echo "doc-budget: 宣言リストが無いため skip: scripts/repository/doc-budget.txt"
  exit 0
fi
...
while IFS= read -r decl || [ -n "$decl" ]; do
  ...
  if [ -z "${max_section:-}" ] || [ -n "${rest:-}" ] ||
    ...
    echo "doc-budget: 宣言が不正: scripts/repository/doc-budget.txt:$lineno: $decl" >&2
...
echo "  超過分は docs/ へ出すか圧縮する。上限値の変更は scripts/repository/doc-budget.txt。"
exit 1
