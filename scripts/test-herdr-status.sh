#!/bin/bash
# .config/herdr/scripts/status.sh のユニットテスト
#
...
TARGET="$SCRIPT_DIR/../.config/herdr/scripts/status.sh"
...
run_target() {
  env \
    ...
    HERDR_STATUS_EPOCH="${HERDR_STATUS_EPOCH:-$EPOCH}" \
...
check_eq "日付・曜日・秒・CPU・MEM・LA が並ぶ" \
  "8/20 Thu 11:42:03 · CPU 50% · MEM 5.9/11.7G · LA 1.06" \
  "$(printf '%s' "$out" | strip_ansi)"

echo "=== 月日はゼロ埋めしない ==="
seed_cache
out="$(HERDR_STATUS_EPOCH="$(date -d '2026-01-05 09:03:04' +%s)" run_target | strip_ansi)"
...
for i in 0 1 2 3 4 5 6; do
  ...
  out="$(LC_ALL=C HERDR_STATUS_EPOCH="$ep" run_target | strip_ansi)"
...
out="$(env \
  ...
  HERDR_STATUS_CACHE="$CACHE" \
  HERDR_STATUS_EPOCH="$EPOCH" \
  HERDR_STATUS_COLOR=never \
  "$TARGET" 2>/dev/null)"
rc=$?
check_eq "読めなくても exit 0" "0" "$rc"
check_eq "時刻だけは出す" "8/20 Thu 11:42:03" "$(printf '%s' "$out" | strip_ansi)"
