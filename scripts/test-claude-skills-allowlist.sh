...
LIB="$SCRIPT_DIR/lib/trusted-skill-owners.sh"
ADD="$SCRIPT_DIR/skill-add.sh"
SETUP="$SCRIPT_DIR/setup-claude-skills.sh"
OWNERS="$SCRIPT_DIR/trusted-skill-owners.txt"
...
TOTAL=$((TOTAL + 1))
if grep -q '^local:' "$SCRIPT_DIR/claude-skills.txt"; then
  FAIL=$((FAIL + 1))
